...
export const PacientesRecepcion = () => {
  ...
  const estadoInicial = { 
    id: null, fullName: '', identityDocument: '', dateOfBirth: '', 
    gender: 'female', phone: '', address: '', esMenor: false 
  };
  ...
  const handleAbrirModal = (paciente = null) => {
    ...
    if (paciente) {
      setFormData({
        ...
        dateOfBirth: paciente.yearOfBirth ? paciente.yearOfBirth.split('T')[0] : '',
  ...
  const handleGuardar = async (e) => {
    ...
    try {
      ...
      setErrorMsg(error.response?.data?.message || "Ocurrió un error en el servidor.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const pacientesFiltrados = pacientes.filter(p => 
    ...
    p.identityDocument.includes(busqueda) ||
    p.fileNumber.includes(busqueda)
  );
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '1100px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        <button onClick={() => handleAbrirModal()} className="submit-btn" style={{ width: 'auto', padding: '0.8rem 1.5rem', margin: 0 }}>
          + Nuevo Paciente
        </button>
      </div>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <input 
          type="text" placeholder="Buscar por nombre, DUI o expediente..." className="form-input"
            ...
            {pacientesFiltrados.map((p) => (
              <tr key={p.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                <td style={{ padding: '1.2rem 1.5rem' }}>
                  ...
                  <div style={{ fontSize: '0.85rem', color: '#0d9488', fontWeight: 'bold' }}>{p.fileNumber}</div>
                </td>
                ...
                <td style={{ padding: '1.2rem 1.5rem' }}>
                    <span style={{ 
                      ...
                      backgroundColor: p.isMinor ? '#ecfdf5' : '#eff6ff', 
                      color: p.isMinor ? '#059669' : '#2563eb' 
                    }}>
      ...
      {mostrarModal && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          <div style={{ backgroundColor: 'white', padding: '2.5rem', borderRadius: '1.2rem', width: '90%', maxWidth: '650px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <form onSubmit={handleGuardar} className="login-form">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.2rem' }}>
                ...
                <div className="form-group">
                  <label className="form-label">
                    {formData.esMenor ? 'DUI del Responsable' : 'DUI del Paciente'}
                  </label>
                  <input 
                    type="text" className="form-input" placeholder="00000000-0" maxLength="10" required 
                    value={formData.identityDocument} onChange={handleDUIChange} 
                  />
                </div>
              ...
              <div className="form-group" style={{ marginTop: '1rem' }}>
                ...
                <textarea className="form-input" rows="2" style={{ padding: '0.8rem' }} value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})}></textarea>
              </div>

              {errorMsg && (
                <div style={{ color: '#ef4444', backgroundColor: '#fee2e2', padding: '10px', borderRadius: '8px', textAlign: 'center', marginTop: '1.5rem', fontWeight: 'bold' }}>
                  {errorMsg}
                </div>
              )}
